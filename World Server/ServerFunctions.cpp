...
bool CWorldServer::NPCShout( int map, char msg[200], char npc[50] )
{
...
{   //if droptype = 1 then it is a normal drop. if it is 2 then it is a potential side drop.
    ...
    if( newdrop->item.itemtype == 10 || newdrop->item.itemtype == 12 )
    {
        ...
        if(newdrop->item.count == 0)
            newdrop->item.count = 1;
        if(newdrop->item.type == 428) //bindrunes and talismans
            newdrop->item.count = RandNumber(0, 5);
...
*/
    ...
    int factor = 100 + (5 * dif);
    //Log(MSG_INFO,"Level dif = %i XP factor is %i",dif,factor);
    if(dif>15)//purple
        exp = (UINT)floor(exp * 1.75);
    if(dif<-15)//white
        exp = (UINT)floor(exp * 0.25);
    if(dif >=-15 && dif <= 15)
        exp = (UINT)floor(exp * factor / 100);
    //else if(dif>10)
