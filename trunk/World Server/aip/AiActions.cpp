...
AIACT(011)
{
	...
	for(UINT j=0;j<map->MonsterList.size();j++)
    {
        ...
        //		int dY = (int)abs((int)other->curBlock.y - (int)entity->curBlock.y);
        //		if(dY > 1) continue;
        int iDistance = GServer->distance( other->Position->current, entity->Position->current );
        //		int iDistance = other->basic.pos.distance(entity->basic.pos);
		if(iDistance > searchDistance) continue;
		chrCount++;
...
AIACT(029)
{
	...
	ADDWORD    ( pak, 0xcdcd );
	ADDBYTE    ( pak, thisMonster->thisnpc->stance );
	GServer->SendToVisible(&pak, entity);
...
AIACT(031)
{
	...
	if(target == NULL) return AI_FAILURE;
	// need to prevent the summon from attacking other summons of teh same player
