...
bool CWorldServer::pakDoIdentify( CPlayer *thisclient, CPacket *P )
{
	...
	ADDBYTE    ( pak, 0 );
	thisclient->client->SendPacket( &pak );
	SendSysMsg( thisclient, "OsProse server made by the comunity of osrose " );
	thisclient->SetStats( );
	return true;
}
...
bool CWorldServer::pakModifiedItem( CPlayer* thisclient, CPacket* P )
{
    ...
    switch(action)
    {
        ...
        case 0x03: //itemdivide by npc
        {
            ...
            SendPM(thisclient, "Material lookup number = %i", material_lookup_number);
            ...
            if(matType != 0)
            {
                ...
                SendPM(thisclient, "item type = %i", matType);       
            }
            else
            {
